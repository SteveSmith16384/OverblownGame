...
public class MyModelLoader extends ModelLoader<ModelLoader.ModelParameters> {
	...
	protected void parseMeshes (ModelData model, JsonValue json) {
		...
		if (meshes != null) {
			...
			for (JsonValue mesh = meshes.child; mesh != null; mesh = mesh.next) {
				...
				int count = 0;
				for (int i=0 ; i<jsonMesh.attributes.length ; i++) {
					count += jsonMesh.attributes[i].numComponents;
				}
				...
				if (keyframes != null && keyframes.isArray()) {
					for (JsonValue keyframe = keyframes.child; keyframe != null; keyframe = keyframe.next) {
						...
						if (translation != null && translation.size == 3) {
							...
							//tkf.value = new Vector3(translation.getFloat(0)*model_scale, translation.getFloat(1)*model_scale, translation.getFloat(2)*model_scale);
