...
class VoxelFileReader {
	...
	void read(FileHandle path,
			  SizeReader onSize,
			  VoxelReader onVoxel,
			  PaletteReader onPalette) throws IOException {

		BufferedInputStream input = path.read(1024);
		Chunk chunk = new Chunk();

		if (read32(input) != magicValue('V', 'O', 'X', ' ')) {
			throw new RuntimeException("Not a valid .vox file.");
		}
		...
		if (chunk.id != magicValue('M', 'A', 'I', 'N')) {
			throw new RuntimeException("Main chunk expected.");
		}

		if (input.skip(chunk.contentSize) != chunk.contentSize) {
			throw new RuntimeException("Invalid number of bytes skipped.");
		}
			...
			if (chunk.id == magicValue('S', 'I', 'Z', 'E')) {
				...
				onSize.size(width, depth, height);
					...
					for (int v = 0; v < numVoxels; v++) {
						...
						onVoxel.voxel(x, y, z, (byte) (c & 0xff));
					}
